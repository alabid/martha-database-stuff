...
function addEnergy($data){
  ...
  $query = "SELECT MeasuredValue, EnergyDataID, BTUConversion FROM EnergyData WHERE Date='{$date}' AND Duration='{$duration}' AND MeterID = '{$meterID}' AND Unit='{$unit}'";
  $res = mysql_query($query);
  if ($row = mysql_fetch_array($res)){
    ...
    if ($row["MeasuredValue"]!=$measuredValue && $measuredValue!==null && $measuredValue!==""){
      $id = $row["EnergyDataID"];
      $query = "UPDATE EnergyData SET MeasuredValue={$measuredValue} WHERE EnergyDataID={$id}";
      mysql_query($query);
    }
    return;
