...
function optionHandler(selection,id){
    ...
    else if (selection.value.toString() == "building" || selection.value.toString() == "fuelType"){
	var url = "dropdown.php?";
	//alert(url);
	var constraints = parseInt(document.getElementById("constraints").value);

	for (var i=0;i<constraints;i++){
	    if (document.getElementById('consSel'+i)==null){
		break;
	    }
	    var temp =  document.getElementById('consSel'+i).value;
	    if (temp!=null && document.getElementById(temp)!=null && document.getElementById(temp).value!=null ){
		url += temp+"=";
		url += document.getElementById(temp).value+"&";
	    }
	}
	if (selection.value.toString() == "building"){
